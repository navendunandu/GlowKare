...
if(isset($_POST["btn_register"]))
{
	...
	$tmplogo=$_FILES['file_logo']['tmp_name'];
	move_uploaded_file($tmplogo, '../Assets/Files/Seller/Logo/'.$logo);
	$proof=$_FILES['file_proof']['name'];
	$tmpproof=$_FILES['file_proof']['tmp_name'];
	move_uploaded_file($tmpproof, '../Assets/Files/Seller/Proof/'.$proof);
	...
	if($resUser->num_rows>0 || $resAdmin->num_rows>0 || $resDerma->num_rows>0){
	  ...
	  if($confirmpassword!=$password)
      {
        ...
        $InsQry = "insert into tbl_seller(seller_name,seller_email,seller_address,place_id,seller_logo,seller_proof,seller_password,seller_doj,seller_contact)values('$name','$email','$address','$place','$logo','$proof','$password',curdate(),'$contact')";
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea required class="form-control" name="txt_address" id="txt_address" rows="3"></textarea>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input class="form-control" required type="file" name="file_logo" id="file_logo" />
                                    </div>
                                    <div class="mb-3">
                                        <label for="file_proof" class="form-label">Proof</label>
                                        <input class="form-control" required type="file" name="file_proof" id="file_proof" />
                                    </div>
                                    <div class="mb-3">
                                        <label for="txt_password" class="form-label">Password</label>
                                        <input type="password"  class="form-control" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" required name="txt_password" name="txt_password" id="txt_password" />
                                    </div>
                                    <div class="mb-3">
                                        <label for="txt_confirm" class="form-label">Confirm Password</label>
                                        <input required type="password" class="form-control" name="txt_confirm" id="txt_confirm" />
                                    </div>
                                </div>
                            </div>
                            <div class="row">
